...
def ensure_vpn(provider: str) -> None:
    selected_providers = [p for p in providers if p.name.lower() == provider.strip().lower()]
    if len(selected_providers) != 1:
        raise EnsureVPNException(f"No or too many VPN providers found (results: {selected_providers})")
    ...
    if is_successful(wrapped_result):
        ...
        if (result == False):
            raise VPNNotConnectedException(f"You are not connected to {selected_provider.name}.")
